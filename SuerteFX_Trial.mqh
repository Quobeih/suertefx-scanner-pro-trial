...
#define SFX_GV_TS     "SFX_TRIAL_TS"
#define SFX_MAX_AGE_S 1800   // GV must be refreshed within 30 minutes
...
int SFX_CheckTrial()
{
   if(!GlobalVariableCheck(SFX_GV) || !GlobalVariableCheck(SFX_GV_TS))
   {
   ...
   // Reject stale GlobalVariables — a manually-set GV without the EA won't have a fresh timestamp.
   datetime ts = (datetime)GlobalVariableGet(SFX_GV_TS);
   if((int)(TimeGMT() - ts) > SFX_MAX_AGE_S)
   {
      Print(SFX_PRODUCT, ": License data is stale — EA may not be running. Pausing.");
      SFX_DrawSetupPanel();
      return -1;
   }
